...
const UpcomingEvents = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    e.preventDefault(); // This is the magic line that stops text selection!
    const x = e.pageX - scrollRef.current.offsetLeft;
    const walk = (x - startX) * 2; // The * 2 determines scroll speed (drag sensitivity)
    scrollRef.current.scrollLeft = scrollLeft - walk;
  };

  return (
    <section className="max-w-[1400px] mx-auto px-4 py-16 bg-white overflow-hidden">
      ...
      <div className="text-center mb-8 flex items-center justify-center gap-2 text-gray-400 text-sm select-none">
      ...
      <div 
        ...
        // Removed snap-mandatory while dragging to make the drag feel smoother
        className={`flex flex-nowrap overflow-x-auto gap-6 md:gap-8 pb-10 pt-2 px-2 events-scrollbar items-stretch ${
      ...
      >
        {eventsData.map((event) => (
          <div 
            key={event.id} 
            // Added select-none so the text inside doesn't highlight while dragging
            className="w-[85vw] sm:w-[400px] md:w-[450px] shrink-0 snap-center group flex flex-col bg-white rounded-xl shadow-sm hover:shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 transform hover:-translate-y-1 select-none"
          >
            {/* Image Container */}
            <div className="relative h-56 w-full overflow-hidden bg-gray-50 flex-shrink-0">
              <img 
                src={event.image} 
                alt={event.title} 
                // pointer-events-none stops the image from accidentally being "dragged" by the browser
            ...
            <div className="p-6 flex flex-col flex-1 bg-white">
              <h3 className="text-lg font-bold text-[#004b87] mb-3 uppercase tracking-wide group-hover:text-orange-500 transition-colors duration-300">
                {event.title}
              </h3>
              
              {/* Delicate divider */}
              <div className="w-10 h-[2px] bg-orange-400/70 mb-4 transition-all duration-300 group-hover:w-16"></div>
      ...
      <style>{`
        /* Styles for the events horizontal scrollbar */
        .events-scrollbar::-webkit-scrollbar {
          height: 8px; /* Height of horizontal scrollbar */
        }
        .events-scrollbar::-webkit-scrollbar-track {
          background: #f1f5f9; /* Soft gray track */
          border-radius: 10px;
        }
        .events-scrollbar::-webkit-scrollbar-thumb {
          background: #cbd5e1; /* Neutral thumb */
          border-radius: 10px;
        }
        .events-scrollbar::-webkit-scrollbar-thumb:hover {
          background: #fb923c; /* Orange-400 on hover to match theme */
        }
        
        /* Hide scrollbar on mobile for cleaner swipe, keep on desktop */
...
////// IF YOU WANT HORIZONTAL SCROLL UNCOMMENT THE BELOW AND COMMENT THE ABOVE SECTION 
...
//       <div className="relative bg-white rounded-2xl border border-gray-100 shadow-sm p-2 md:p-6">
        
//         {/* Scroll Box: max-h limits the height so it doesn't stretch the home page */}
//         <div className="max-h-[650px] overflow-y-auto custom-scrollbar pr-3 overscroll-contain">
          
//           {/* The Original 3-Column Grid */}
//           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 pb-8">
//             {eventsData.map((event) => (
//               <div 
//                 key={event.id} 
//                 className="group flex flex-col bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 overflow-hidden transition-all duration-300 transform hover:-translate-y-1"
//               >
//                 {/* Image Container */}
...
//                   {/* Delicate divider */}
//                   <div className="w-10 h-[2px] bg-blue-400/70 mb-4 transition-all duration-300 group-hover:bg-orange-400 group-hover:w-14"></div>
...
//         {/* Premium Bottom Fade Effect (Visual cue that there is more to scroll) */}
//         <div className="absolute bottom-0 left-0 w-full h-16 bg-gradient-to-t from-white to-transparent pointer-events-none rounded-b-2xl"></div>
//       </div>

//       {/* Custom Vertical Scrollbar Styles */}
//       <style>{`
//         .custom-scrollbar::-webkit-scrollbar {
//           width: 8px; /* Slightly thicker for vertical scrolling */
//         }
...
//           background: #fb923c; /* Orange hover state */
//         }
//       `}</style>
//     </section>
//   );
